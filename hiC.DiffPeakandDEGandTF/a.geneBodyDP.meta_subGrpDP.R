...
files.in.diffPeak.RData = paste0("~/lhou.compbio/codes/MayoBipolar_2019_10/peakMergeNormal/c_bulkDiffPeak_limma_V1.5.1_inflamSubgrp/", 
                                HMs, "/", HMs, ".diffPeak.RData")
...
dir.ou="a_geneBody_DP_meta_V1.2.1_filtByDist_allGenes/DP.V1.5.1_inflamSubgrp/"
dir.create(dir.ou, showWarnings = F, recursive = T)
#file.ou.pkAndGeneBody.RDS = paste0(dir.ou, "pkAndGeneBody.RDS")
file.ou.RData=paste0(dir.ou, SUBGRP, ".genebody.DP.meta.RData")
file.ou.pdf=paste0(dir.ou, SUBGRP, ".genebody.DP.meta.allGenes.pdf")
...
FUN=function(HM)
{
  ...
  dp.signals=cbind(log2FoldChange= res[[SUBGRP]]$logFC, 
                   pvalue=res[[SUBGRP]]$P.Value, 
                   z = sign(res[[SUBGRP]]$logFC)* abs(qnorm(res[[SUBGRP]]$P.Value/2)),
                   padj = res[[SUBGRP]]$adj.P.Val)
  rownames(dp.signals) = rownames(res[[SUBGRP]])
...
for(nm in names(gENS.list))
{
  ...
  write(gName, file=paste0(dir.ou, SUBGRP, ".", nm, ".q", DEG.q.cutoff, ".geneSymbol.txt"))
  write(esmGid.v2esmGid[gENS.list[[nm]]], file=paste0(dir.ou, SUBGRP, ".", nm, ".q", DEG.q.cutoff, ".ENSG.txt"))
}
