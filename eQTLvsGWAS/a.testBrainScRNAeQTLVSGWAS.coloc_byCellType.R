...
CELLTYPE = args[1] #CELLTYPEs = c("Astro", "Ex-L4", "Ex-NRGN", "In-PV-Chandelier", "In-SST", "In-VIP", "OPC", "Ex-L2or3", "Ex-L5or6", "In-PV-Basket", "In-Rosehip", "In-SV2C", "Microglia", "Oligo")
...
dir.in.eQTL = paste0("/broad/compbio/lhou/data/QTL/Brain.scRNA.eQTL.fromYongjin/bed.hg38/", CELLTYPE, "/")
...
dir.tmp=paste0("~/hptmp/MayoBipolar/a_Brain.scRNA.eQTLvsGWAS.coloc/")
...
dir.ou = paste0("a_BrainscRNA.eQTLvsGWAS_bycoloc/", CELLTYPE, "_", GENE.WIND/1000, "k/")
...
cmd=paste0("zcat ", dir.in.eQTL, "*.gz |awk '($8<=", EQTL.P.CUTOFF, "){print $1 \"\\t\" $2 \"\\t\" $3 \"\\t\" $4}' >", file.tmp.eQTL.bed )
system(cmd)
...
FUN=function(gene)
{
  ...
  file.in.eQTL = paste0(dir.in.eQTL, g.chr, "_", CELLTYPE, ".sorted.bed.gz")
  #g.end = eGenes.df[gene, "gene_end"]
  eQTL.tab = seqminer::tabix.read.table(file.in.eQTL, query) 
  eQTL.tab.filt = eQTL.tab[eQTL.tab$ensembl_gene_id==gene,]
  eQTL.tab.filt = eQTL.tab.filt[!duplicated(eQTL.tab.filt$id),]
  rownames(eQTL.tab.filt) = eQTL.tab.filt$id
  eQTL.tab.filt$varbeta=(eQTL.tab.filt$se)^2
  #
  eQTL.tab.filt.flip = eQTL.tab.filt
  eQTL.tab.filt.flip$a1= eQTL.tab.filt$a0
  eQTL.tab.filt.flip$a0= eQTL.tab.filt$a1
  rownames(eQTL.tab.filt.flip)  = paste(eQTL.tab.filt.flip$chr, eQTL.tab.filt.flip$ub, eQTL.tab.filt.flip$a1, eQTL.tab.filt.flip$a0, sep="_")
  ...
  if(length(snp.loc.ovlp.kept)==0) next
  ...
  df =data.frame(log10p= c(-log10(gwas.tab.all[snp.loc.ovlp.kept, "p"]),
                           -log10(eQTL.tab.filt.all[snp.loc.ovlp.kept, "p.val"])),
