...
SCRIPT= "a.2.linkGenePeak_MR-Egger_perGene.R"
...
HM = argv[2]
TISS = "Blood" # argv[3]
hQTL.WIND = 100000 #as.numeric(argv[4])
...
#file.in.g2pk.RData = paste0("./a_AREsNeareQTL/", TISS, "_10k/", HM, ".gene2peaks_byeQTL.hg38.RData")
file.in.g2pk.RData = paste0("./a_gAREsNeareGene_emp.p.fdr.cutoff0.2/", TISS, "_1000k/", HM, ".eGene2gARE.hg38.RData")

dir.tmp =paste0("~/hptmp/MayoBipolar/GeneEnhLink_MR-egger_V1.1.1/", TISS, "_", HM, "_", hQTL.WIND/1000, "k/")
...
# file.tmp.eQTLOVLPpks.hg38.bed = paste0(dir.tmp, TISS, ".eQTLOVLPpeaks.hg38.bed")
 #paste0(dir.tmp, TISS, ".gene2peaks.hg38.RData")
...
for(JOB.I in 1:JOBs.N)
{
  ...
  file.tmp.link.RData  = paste0(dir.tmp, TISS, ".", HM, ".geneEnhLink_", JOB.I.inFN,".RData")
  
  if(MOD == "rerun" && file.exists(file.tmp.link.RData))
  {
    next
  }
  
  f.o.sh = paste0("a.2.link_MR.", TISS, ".", HM, ".",JOB.I.inFN, ".sh")
  write(sh.heads[MOD], f.o.sh)
    
  write(paste("Rscript ", SCRIPT, TISS, HM, file.in.g2pk.RData, file.tmp.link.RData, hQTL.WIND, JOB.I, JOBs.N, sep=" "), f.o.sh, append = T) #a.linkGenePeak_perGene_PRS.R
