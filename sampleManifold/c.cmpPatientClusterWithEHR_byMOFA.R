#V1.1.2
#c.cmpPatientClusterWithEHR_byMOFA_V1.1.2_addNumericFeature.R
...
# dir.ou="c_patientClu2EHR_byMOFA/"
# dir.create(dir.ou, showWarnings = F, recursive = T)

file.ou.RData = gsub("b_factorAnalysis_across5HMs_MOFA", "c_patientClu2EHR_5HMs_byMOFA", file.in.clu.RData)
...
patient2clu= samp2grp[samp2grp$group=="case", CLU.RES]
names(patient2clu) = rownames(samp2grp)[samp2grp$group=="case"]
...
for(type in names(EHR))
{
  ...
  controls.filt = setdiff(samps.ovlp, patients) 
  ...
  for(grp in names(grp2patients))
  {
    ...
    for(term in names(term2samp))
    {
      ...
      # patient group vs all controls
      samps.grpAndctrls=union(patients.grp.filt, controls.filt)

      samps.term.filt =  intersect(samps.grpAndctrls, term2samp[[term]])
      samps.TermAndDis = length(intersect(samps.term.filt, patients.grp.filt))
      samps.termSpec = length(setdiff(samps.term.filt, patients.grp.filt))
      samps.disSpec = length(setdiff(patients.grp.filt, samps.term.filt))
      samps.bg = length(setdiff(samps.grpAndctrls, union(samps.term.filt, patients.grp.filt)))
      
      fisher.res2 = fisher.test(matrix(c(samps.TermAndDis, samps.termSpec, samps.disSpec, samps.bg), ncol=2), alternative="greater")
      ...
      EHR.enrichment[[type]]=rbind(EHR.enrichment[[type]],
                                   data.frame(patientGroup = grp,
                                              ...
                                              grp.vs.ctrl.OR = fisher.res2$estimate,
                                              grp.vs.ctrl.OR.pval = fisher.res2$p.val,
                                              stringsAsFactors = F)
      )
    }
  }
...
print(ncol(clinic))
patients.ovlp= intersect(names(patient2clu), rownames(clinic))
clinic$newsubjectid=NULL  
EHR.enrichment[["clinic"]]=data.frame()
for(grp in names(grp2patients))
{
  patients.grp.filt = intersect(patients.ovlp, grp2patients[[grp]]) 
  if(length(patients.grp.filt)<3) next
  patients.othGrp.filt=setdiff(patients.ovlp, patients.grp.filt)
  
  for(term in colnames(clinic))
  {
    clinic[[term]][clinic[[term]]==""]=NA
    if(sum(is.na(clinic[[term]]))>=0.4*nrow(clinic)) next
    
    if(length(levels(factor(clinic[[term]])))==2)
    {
      ctable= as.matrix(table(data.frame(grp=factor(patients.ovlp %in% patients.grp.filt, levels=c("TRUE", "FALSE")),
                               term=clinic[patients.ovlp, term])))
      if(colnames(ctable)[2] == "1" || colnames(ctable)[2]=="Yes" || colnames(ctable)[2]=="Pos")
      {
        ctable=ctable[,c(2,1)]
      }
      ...
      res = fisher.test(ctable, alternative="greater")
      
      
      EHR.enrichment[["clinic"]]=rbind(EHR.enrichment[["clinic"]],
                                   data.frame(patientGroup = grp,
                                              EHR.term= term,
                                              grp.OR=res$estimate,
                                              grp.OR.pval=res$p.value,
                                              total.OR = 1,
                                              total.OR.pval = 1,
                                              stringsAsFactors = F))
    }else
    {
      if(class(clinic[[term]]) =="numeric")
      {
        x.grp=clinic[patients.grp.filt,term]
        x.othGrp=clinic[patients.othGrp.filt,term]
        res=t.test(x.grp, x.othGrp, alternative="greater")
        
        EHR.enrichment[["clinic"]]=rbind(EHR.enrichment[["clinic"]],
                                         data.frame(patientGroup = grp,
                                                    EHR.term= term,
                                                    grp.OR=res$statistic,
                                                    grp.OR.pval=res$p.value,
                                                    total.OR = 1,
                                                    total.OR.pval = 1,
                                                    stringsAsFactors = F))
...
#grp2samps.list$clu6= split(names(patient2clu.list$clu6), paste0("grp", patient2clu.list$clu6))



#for mixEHR, wilcox test
mixEHR.mod=read.table(file.in.mixEHR, sep="\t", head=T, row.names=1)
samps.ehr =rownames(mixEHR.mod)
samps.ovlp = intersect(samps.ehr, samps.mofa)
patients = names(patient2clu)
patients.filt = intersect(samps.ovlp, patients) 

EHR.enrichment[["mixEHR"]]=data.frame()
for(grp in names(grp2patients))
{
  patients.grp.filt = intersect(samps.ovlp, grp2patients[[grp]]) 
  #patients.grp.filt =intersect(patients.filt, patients.grp.filt)
  patients.othGrp.filt=setdiff(patients.filt, patients.grp.filt)
  for(mod in colnames(mixEHR.mod))
  {
    
    scores.grpPatient=mixEHR.mod[patients.grp.filt, mod]
    scores.othPatients=mixEHR.mod[patients.othGrp.filt, mod]
    scores.patients=mixEHR.mod[patients.filt, mod]
    scores.controls=mixEHR.mod[setdiff(samps.ovlp, patients.filt), mod]
    
    #patients
    res1=ks.test(scores.grpPatient, #non-parameteric test
                              scores.othPatients,
                              alternative= "greater")
    
    #whole samples
    res0=ks.test(scores.patients, #non-parameteric test
                             scores.controls,
                             alternative= "greater")
    
    
    EHR.enrichment[["mixEHR"]]=rbind(EHR.enrichment[["mixEHR"]],
                                 data.frame(patientGroup = grp,
                                            EHR.term= mod,
                                            grp.diff=res1$statistic,
                                            grp.pval=res1$p.value,
                                            total.diff =res0$statistic,
                                            total.pval = res0$p.value,
                                            stringsAsFactors = F))
  }
}
...
ht1=Heatmap(t(mofa.fators.impute.scaled[patients,]), 
            column_split = paste0("grp", samp2grp[patients, "clu.corHclust5"]),
...
#
f.o.heatmap.tsv=gsub("RData", "heatmap.tsv", file.ou.RData)
write.table(t(EHR.terms.slct.matrix.filt), f.o.heatmap.tsv, sep="\t", quote=F)
